...
`cad_interfaz_cti` de la base de datos local, para que el operador siga
viendo llamadas entrantes por el mismo mecanismo de polling que usa en modo
normal.

## Qué hace

- `POST /api/cti` — recibe `{ acd, numTelefono, sitioGraba }`, valida
  `X-Api-Key` e inserta una fila en `cad_interfaz_cti` (BD local).
...
instancia PostgreSQL standalone con el esquema de `cad_interfaz_cti`) a la
...
- `EdgeCti:SitioGraba` es el sitio de grabación por defecto de esta sede; se
...
```json
{ "success": true, "message": "Llamada CTI registrada en el nodo edge (modo offline).", "id": "123" }
```

Es intencionalmente el mismo contrato que
`POST api/RecepcionExterna/cti` del backend central, para que la PBX pueda
usar la misma plantilla de payload en ambos destinos (primario y fallback).

## Despliegue

```bash
docker build -t edge-cti-service -f backend/edge-cti-service/Dockerfile backend/edge-cti-service
docker run -p 8081:8081 \
  -e ConnectionStrings__LocalDb="Host=...;Database=...;Username=...;Password=..." \
  -e EdgeCti__ApiKey="..." \
  -e EdgeCti__SitioGraba=1 \
  edge-cti-service
```
